...
uintptr_t g_selected_node = 0x0;
RBXInstance dm(get_data_model());

void render_tree(RBXInstance& node)
{
	ImGuiTreeNodeFlags node_flags = ImGuiTreeNodeFlags_OpenOnArrow;
	if (node.get_self() == g_selected_node)
	...
	if (has_children)
	{
		if (ImGui::TreeNodeEx(node.get_name().c_str(), node_flags))
		{
	...
	{
		if (ImGui::Selectable(node.get_name().c_str(), node.get_self() == g_selected_node))
			g_selected_node = node.get_self();
	}
...
void Overlay::Render()
{
	ImGui::Begin("Explorer", &RenderMenu, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar);

	ImGui::Columns(2, "explorer_columns", true);

	render_tree(dm);

	ImGui::NextColumn();

	if (g_selected_node != 0x0)
	{
		RBXInstance node(g_selected_node);

		ImGui::Text("class name: %s", node.get_class_name().c_str());
		ImGui::Text("name: %s", node.get_name().c_str());
		ImGui::Text("image: 0x%llX", node.get_self());
